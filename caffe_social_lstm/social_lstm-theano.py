...
class data_layer(caffe.Layer):
	def setup(self, bottom, top):
		...
		'hidden_data', 'cell_data']
		self.params = eval(self.param_str)
		if self.params['phase'] == 'train':
			self.params['data_dirs'] = ['D:\\workspace\\sociallstm\\social-lstm-tf-master\\data\\eth\\univ']
...
#		drho = T.grad(result, rho)
#
#		f = theano.function([mux, muy, sx, sy, rho, x, y], result)
#		dfmux = theano.function([mux, muy, sx, sy, rho, x, y], dmux)
#		dfmuy = theano.function([mux, muy, sx, sy, rho, x, y], dmuy)
#		dfsx = theano.function([mux, muy, sx, sy, rho, x, y], dsx)
#		dfsy = theano.function([mux, muy, sx, sy, rho, x, y], dsy)
#		dfrho = theano.function([mux, muy, sx, sy, rho, x, y], drho)
#
#		#fsx = theano.function([sx], z_sx)
#		#fsy = theano.function([sy], z_sy)
#		#fcorr = theano.function([rho], z_corr)

		bottom0 = np.array(bottom[0].data)
		bottom1 = np.array(bottom[1].data)
		imux, imuy, isx, isy, irho = bottom0.T
		ix, iy = bottom1.T
		imux, imuy, isx, isy, irho, ix, iy = imux.reshape(bottom0.shape[0]), imuy.reshape(bottom0.shape[0]), isx.reshape(bottom0.shape[0]), isy.reshape(bottom0.shape[0]), irho.reshape(bottom0.shape[0]), ix.reshape(bottom0.shape[0]), iy.reshape(bottom0.shape[0])

		top[0].data[...] = self.f(imux, imuy, isx, isy, irho, ix, iy)
		#logging.warning("loss {}".format(f(imux, imuy, isx, isy, irho, ix, iy)))
		#logging.warning('{} {} {} {} {}, {} {}\n'.format(imux[0], imuy[0], fsx(isx)[0], fsy(isy)[0], fcorr(irho)[0], ix[0], iy[0]))
#		self.muxdiff = dfmux(imux, imuy, isx, isy, irho, ix, iy)
#		self.muydiff = dfmuy(imux, imuy, isx, isy, irho, ix, iy)
#		self.sxdiff = dfsx(imux, imuy, isx, isy, irho, ix, iy)
#		self.sydiff = dfsy(imux, imuy, isx, isy, irho, ix, iy)
#		self.rhodiff = dfrho(imux, imuy, isx, isy, irho, ix, iy)

#		top[0].data[...] = self.f(imux, imuy, isx, isy, irho, ix, iy)
#		self.muxdiff = self.dfmux(imux, imuy, isx, isy, irho, ix, iy)
#		self.muydiff = self.dfmuy(imux, imuy, isx, isy, irho, ix, iy)
#		self.sxdiff = self.dfsx(imux, imuy, isx, isy, irho, ix, iy)
#		self.sydiff = self.dfsy(imux, imuy, isx, isy, irho, ix, iy)
#		self.rhodiff = self.dfrho(imux, imuy, isx, isy, irho, ix, iy)
